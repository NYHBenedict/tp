...
Typical usage:
* Use `viewall` after adding or removing assessments to confirm that the total number of assessments has changed as expected.
* Use `viewall` after adding or removing grades to quickly verify that the overall grade count has been updated.
* Use `viewall` when you want a quick summary without switching through multiple list commands manually.

Notes:
* `viewall` is intended as a lightweight overview command rather than a full detailed report.
* Depending on the current UI state, the command may work together with the displayed panel to provide a clearer summary of stored records.
...
3. Some commands may present their information mainly through the currently visible panel, so users should check both the result box and the main panel after executing a command.
